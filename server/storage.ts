import {
  ...
  type Environment, type InsertEnvironment, environments,
  type FeatureFlag, type InsertFeatureFlag, featureFlags,
  type FlagState, type InsertFlagState, flagStates,
  type AuditEvent, type InsertAuditEvent, auditEvents,
  type FlagWithStates, type AuditEventFull, type DashboardStats,
} from "@shared/schema";
...
export class DatabaseStorage implements IStorage {
  ...
  async getFlags(projectId: number): Promise<FlagWithStates[]> {
    const flags = db.select().from(featureFlags)
      .where(eq(featureFlags.projectId, projectId))
      .orderBy(desc(featureFlags.createdAt))
      .all();

    const envs = db.select().from(environments)
    ...
    for (const flag of flags) {
      ...
      for (const env of envs) {
        const state = db.select().from(flagStates)
      ...
      const author = flag.authorId
        ? db.select().from(users).where(eq(users.id, flag.authorId)).get()
        : undefined;
  ...
  async getFlag(id: number): Promise<FlagWithStates | undefined> {
    ...
    const envs = db.select().from(environments)
      .where(eq(environments.projectId, flag.projectId))
      .all();
  ...
  async getFlagState(flagId: number, environmentId: number): Promise<FlagState | undefined> {
    return db.select().from(flagStates)
  ...
  async getAuditEvents(_projectId?: number, limit: number = 50): Promise<AuditEventFull[]> {
    ...
    return events.map(event => {
      const actor = event.actorId
        ? db.select().from(users).where(eq(users.id, event.actorId)).get()
        : undefined;
  ...
  async getDashboardStats(projectId: number): Promise<DashboardStats> {
    const flags = db.select().from(featureFlags)
      .where(eq(featureFlags.projectId, projectId)).all();
    const envs = db.select().from(environments)
      .where(eq(environments.projectId, projectId)).all();
    ...
    if (prodEnv) {
      const activeStates = db.select({ cnt: count() }).from(flagStates)
        .where(and(
          eq(flagStates.environmentId, prodEnv.id),
          eq(flagStates.isEnabled, true)
        )).get();
    ...
    const envStats = [];
    for (const env of envs) {
      const activeResult = db.select({ cnt: count() }).from(flagStates)
        .where(and(
          eq(flagStates.environmentId, env.id),
          eq(flagStates.isEnabled, true)
        )).get();
      envStats.push({
        name: env.name,
  ...
  async seed(): Promise<void> {
    ...
    const pm = db.insert(users).values({
    ...
    // Create environments
    const devEnv = db.insert(environments).values({
    ...
    const stagingEnv = db.insert(environments).values({
      projectId: project.id,
      envKey: "staging",
      name: "Staging",
    ...
    const prodEnv = db.insert(environments).values({
      projectId: project.id,
      envKey: "production",
      name: "Production",
    ...
    const seedFlags = [
      ...
      },
      {
        ...
        states: { development: true, staging: true, production: true },
        rules: { production: JSON.stringify([{ type: "user_group", value: "beta-testers" }, { type: "user_group", value: "premium-subscribers" }]) },
        rollout: {},
      },
      {
        ...
        rules: { staging: JSON.stringify([{ type: "percentage", value: 50 }]), production: JSON.stringify([{ type: "percentage", value: 5 }]) },
    ...
    for (const sf of seedFlags) {
      const flag = db.insert(featureFlags).values({
        projectId: project.id,
        authorId: pm.id,
        flagKey: sf.flagKey,
        name: sf.name,
